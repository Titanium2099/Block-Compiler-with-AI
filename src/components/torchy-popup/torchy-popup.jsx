...
class TorchyPopupComponent extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            isOpen: true,
        ...
        this.popupContainerRef = React.createRef(); // Rename the ref
    ...
    handleMouseDown(e) {
        this.setState({
            ...
            hasMoved: false,
            offset: {
                x: e.clientX - this.popupContainerRef.current.offsetLeft, // Use the renamed ref
                y: e.clientY - this.popupContainerRef.current.offsetTop, // Use the renamed ref
            },
        }, () => {
            this.popupContainerRef.current.style.cursor = 'grabbing'; // Use the renamed ref
        });
    }
    ...
    handleMouseMove(e) {
        if (this.state.isDragging) {
            ...
            const divWidth = this.popupContainerRef.current.offsetWidth; // Use the renamed ref
            const divHeight = this.popupContainerRef.current.offsetHeight; // Use the renamed ref
            const tolerance = 10;
    ...
    handleMouseUp() {
        this.setState({ isDragging: false }, () => {
            if (this.popupContainerRef.current) { // Check if containerRef.current is not null
                this.popupContainerRef.current.style.cursor = 'default'; // Use the renamed ref
            }
        });
    }
    ...
    render() {
        ...
        const { dragOver, ...otherProps } = this.props; // Destructure dragOver
        ...
        return (
            <div
                ...
                style={{
                    ...
                    left: `${this.state.position.x}px`,
                    top: `${this.state.position.y}px`,
                    border: dragOver ? '2px solid red' : 'none'
                }}
                ref={this.popupContainerRef} // Use the renamed ref
                onMouseDown={this.handleMouseDown}
            >
...
TorchyPopupComponent.propTypes = {
...
TorchyPopupComponent.defaultProps = {
    fileAttached: false,
    fileAttachedText: '',
    inputValue: '',
    X_COORDINATE: 0,
...
export default connect(mapStateToProps)(TorchyPopupComponent);